...
def DateSelector(page, text, elevation=None):
    ...
    DateSelect=ft.ElevatedButton(text=text, icon=ft.icons.CALENDAR_MONTH, on_click=open_date_picker, bgcolor=ft.colors.TRANSPARENT, elevation=elevation)    
    return DateSelect
...
class ToDo_APP:
    def __init__(self, page, user_id):
        ...
        self.Priority= DropDown(hint="Priority", Option1="High", Option2="Medium", Option3="Low", icon=ft.icons.PRIORITY_HIGH , width=150)
        ...
        page.add(ft.Stack(controls=[
            ...
            ft.Column(controls=[
                ft.Container(content=ft.Column(controls=[
                    ft.Row(controls=[Task_container,ft.FloatingActionButton(icon=ft.icons.ADD, on_click=self.Add_Task, bgcolor=ft.colors.TRANSPARENT, elevation=0)], alignment=ft.MainAxisAlignment.CENTER),
    ...
    def Back_To_Login(self, e):
        self.page.clean()  # Assuming this clears the current page
    ...
    def SignUp(self, e):    
        if (self.Username.value == "" 
            ...
            try:
                ...
                Parameters=(self.Username.value,
                    self.Password.value,
                    self.Email.value, 
