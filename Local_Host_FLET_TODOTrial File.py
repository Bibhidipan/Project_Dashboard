...
def PopUp(page, title, message):
    def ClosePopUp(e):
        ...
        page.update()
    Dialogue = ft.AlertDialog(title=ft.Text(title), content=ft.Text(message), actions=[ft.TextButton("Ok", on_click=ClosePopUp)])
    page.dialog = Dialogue
    page.dialog.open = True
    page.update()

def Databse_Connection():
    return pymysql.connect(host="localhost", user="root", password="SQL@2022bibhi", database="todo_list")

def Create_Database():
    try:
        connection = pymysql.connect(host="localhost", user="root", password="SQL@2022bibhi")
...
class ToDo_APP:
    def __init__(self, page):
        ...
        self.Task = ft.TextField(hint_text="Enter The Task", text_align="Left", width=300)
        self.Priority = ft.Dropdown(hint_text="Priority", width=100, options=[ft.dropdown.Option("High"), ft.dropdown.Option("Medium"), ft.dropdown.Option("Low")])
        self.Target_Date = ft.Text("Target Date")

        def Date_Select(e):
            self.Target_Date.value = e.control.value.strftime('%Y-%m-%d')
            self.DateSelect.text = self.Target_Date.value
        ...
        self.DateSelect = ft.ElevatedButton(
            text=self.Target_Date.value, 
            icon=ft.icons.CALENDAR_MONTH, 
            on_click=open_date_picker
        )

        page.add(self.Task, ft.Row([self.Priority, self.DateSelect, ft.FloatingActionButton(icon=ft.icons.ADD, on_click=self.Add_Task)]))
                ...
                content=ft.Container(
                    content=ft.Column(
                        [
                            ...
                            ft.Row([
                                ft.Text(f"Priority: {Priority}", text_align="start"),
                                ft.Dropdown(
                                    width=120,
                                    value=status,
                                    options=[
